...
type LocalKnowledgeChunk = Omit<RetrievalChunk, "score" | "source" | "metadata"> & {
  terms: string[];
};

const LOCAL_KNOWLEDGE: LocalKnowledgeChunk[] = [
  {
...
function scoreChunk(chunk: LocalKnowledgeChunk, queryTerms: string[]) {
...
function extractTerms(rewrittenQuery: string) {
...
export class TsLocalRetrievalAdapter implements RetrievalAdapter {
  readonly id = "ts-local-retrieval-adapter";

  async execute(input: RetrievalExecutionInput): Promise<RetrievalExecutionResult> {
    const startedAt = Date.now();
    const queryTerms = extractTerms(input.rewrite.rewrittenQuery);
    const visibleCorpus =
      input.retrieval.selectedKnowledgeBaseIds.length > 0
        ? LOCAL_KNOWLEDGE.filter((chunk) => input.retrieval.selectedKnowledgeBaseIds.includes(chunk.knowledgeBaseId))
        : LOCAL_KNOWLEDGE;
