...
function buildPresets(items: SampleQuestionReadModel[]): PromptPreset[] {
  const mapped = items
  ...
  return mapped.length > 0 ? mapped : DEFAULT_PRESETS;
}

export function WelcomeScreen({ onSend, isStreaming, onCancel }: WelcomeScreenProps) {
  ...
  React.useEffect(() => {
    ...
    getSampleQuestionReadModel()
      .then((data) => {
        if (!active || !data || data.items.length === 0) return;
        const mapped = buildPresets(data.items);
        if (mapped.length > 0) {
  ...
  return (
    <div className="relative flex min-h-full items-center justify-center overflow-hidden px-4 py-16 sm:px-6">
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -bottom-36 left-[-80px] h-80 w-80 rounded-full bg-gradient-radial from-[#FDE68A]/40 via-transparent to-transparent blur-3xl"
      />
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              ...
              >
                <span className="inline-flex items-center gap-2">
                  <Brain className={["h-3.5 w-3.5", deepThinking ? "text-[#3B82F6]" : ""].join(" ")} />
                  深度思考
                  {deepThinking ? (
